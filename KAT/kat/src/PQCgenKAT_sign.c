...
int main() {
  ...
  sprintf(fn_req, "PQCsignKAT_%ld_%ld.req", CRYPTO_PUBLICKEYBYTES,
          CRYPTO_BYTES);
  if ((fp_req = fopen(fn_req, "w")) == NULL) {
    printf("Couldn't open <%s> for write\n", fn_req);
    return KAT_FILE_OPEN_ERROR;
  }
  sprintf(fn_rsp, "PQCsignKAT_%ld_%ld.rsp", CRYPTO_PUBLICKEYBYTES,
          CRYPTO_BYTES);
  ...
  // Create the nclude <ctype.RESPONSE file based on what's in the REQUEST file
