...
const uint8_t canary[8] = {0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF};
...
static int test_wrong_pk(void) {
  ...
  for (i = 0; i < NTESTS; i++) {
    ...
    if (MUPQ_crypto_sign_open(sm, &mlen, sm, smlen, pk2)) {
      hal_send_str(
          "OK Signature did not verify correctly under wrong public key!\n");
    } else {
      hal_send_str(
          "ERROR Signature did verify correctly under wrong public key!\n");
    }
    hal_send_str("crypto_sign_open DONE.\n");
    hal_send_str("+");
  }
...
int main(void) {
  ...
  hal_send_str("==========================");
  hal_send_str("======= Test Sign ========");
  test_sign();
  hal_send_str("===== Test Wrong PK ======");
  test_wrong_pk();
  hal_send_str("#");
  return 0;
}
